...
export default function Home() {
  ...
  const handleDeleteGor = (id_gor) => {
    axios.delete(`http://localhost:3000/hapuswork/${id_gor}`)
      .then(() => {
        location.reload()
      })
      .catch((error) => {
              ...
              <tbody>
                {currentGor.map((gor, index) => (
                  <tr key={index} className={`hover:bg-gray-100`}>
                    ...
                    <td className="p-4 border-b">{gor.id_gor}</td>
                    <td className="p-4 border-b">{gor.nama_gor}</td>
                    <td className="p-4 border-b">{gor.id_lokasi}</td>
                    <td className="p-4 border-b">{gor.alamat}</td>
                    <td className="p-4 border-b">
                      {`http://localhost:3008/`+gor.image_gor}
                    </td>
                    <td className="p-4 border-b flex">
                      ...
                      <button
                        ...
                        onClick={() => handleDeleteGor(gor.id_work)}
                      >
                        Delete
                      </button>
                    </td>
